...
async function request<T = any>(
  ...
  try {
    ...
    const hint = errorMessage.includes('aborted') || errorMessage.includes('AbortError')
      ? `请求超时（>${backendConfig.timeout}ms）`
      : '网络不可达或后端地址配置错误'
    console.error(`[API请求失败] ${method} ${url} - ${hint}: ${errorMessage}`)
    throw error
  }
}
...
export class ApiClient {
  ...
  static async getGames(
    ...
    requestOptions: Pick<RequestConfig, 'next' | 'cache'> = {}
  ) {
  ...
  static async getHomeData(params: { 
    ...
    return request(apiEndpoints.home, {
      method: 'GET',
      params: prepareParams({
        locale: getCurrentLocale(params.locale as any),
      ...
      next: { revalidate: 300, tags: ['home', 'games', 'articles'] },
    })
  }
}
