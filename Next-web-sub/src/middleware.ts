...
const fallbackSupportedLocales = ['zh-CN', 'zh-TW']
const envSupportedLocales = process.env.NEXT_PUBLIC_SUPPORTED_LOCALES
  ?.split(',')
  .map((item) => item.trim())
  .filter(Boolean)

const supportedLocales = (envSupportedLocales && envSupportedLocales.length > 0)
  ? Array.from(new Set(envSupportedLocales))
  : fallbackSupportedLocales

const envDefaultLocale = process.env.NEXT_PUBLIC_DEFAULT_LOCALE?.trim()
const defaultLocale = envDefaultLocale && supportedLocales.includes(envDefaultLocale)
  ? envDefaultLocale
  : supportedLocales[0]
