...
import { isValidLocale, defaultLocale, type Locale } from '@/config/site/locales'
import ApiClient from '@/lib/api'
import ImageWithFallback from '../../ImageWithFallback'
import { JsonLd, buildBreadcrumbSchema, buildBoxSchema } from '@/components/common/JsonLd'
...
export async function generateMetadata({ params, searchParams }: BoxDetailPageProps): Promise<Metadata> {
  ...
  try {
    ...
    const boxPath = locale === defaultLocale ? `/boxes/${id}` : `/${locale}/boxes/${id}`
    const boxUrl = `${siteConfig.hostname}${boxPath}`
    // canonical 指向主站同页面，避免与主站盒子详情页内容重复判重
    const mainSiteCanonical = `${siteConfig.mainSiteUrl}/boxes/${id}`
    const imageUrl = box.logoUrl || '/default-og-image.jpg'
    const description = box.description || `${t.metaDescPrefix}${box.name}${t.metaDescSuffix}`
    const titleWithPage = currentPage > 1
      ? `${box.name} - 第${currentPage}页${t.metaTitleSuffix}`
    ...
    return {
      ...
      openGraph: {
        ...
        type: 'website',
        images: [{ url: imageUrl, width: 1200, height: 630, alt: box.name }],
      },
      twitter: {
...
async function BoxDetailContent({ locale, id, pageNum }: { locale: Locale; id: number; pageNum: number }) {
  ...
  const boxUrlFull = `${siteConfig.hostname}${locale === defaultLocale ? '' : `/${locale}`}/boxes/${id}`
  const boxSchema = buildBoxSchema({
    name: box.name,
    description: box.description,
    url: boxUrlFull,
    imageUrl: box.logoUrl,
    gameCount: box.gameCount ? Number(box.gameCount) : undefined,
  })
  const breadcrumbSchema = buildBreadcrumbSchema([
    { name: t.home, url: siteConfig.hostname },
    { name: t.boxes, url: `${siteConfig.hostname}${locale === defaultLocale ? '' : `/${locale}`}/boxes` },
    { name: box.name, url: boxUrlFull },
  ])

  return (
    <>
      <JsonLd data={[boxSchema, breadcrumbSchema]} />
