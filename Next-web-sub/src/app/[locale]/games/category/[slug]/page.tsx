...
import { JsonLd, buildCollectionPageSchema, buildBreadcrumbSchema } from '@/components/common/JsonLd'
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold mb-2">
            ...
            {locale === 'zh-TW' ? `全部${categoryName}遊戲` : `全部${categoryName}游戏`}
          </h2>
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const catPath = localeTyped === defaultLocale ? `/games/category/${slug}` : `/${localeTyped}/games/category/${slug}`
  const catUrl = `${siteConfig.hostname}${catPath}`
  return {
    title: isTW
      ? `${category.name}遂戏推薦_2026最新${category.name}手遂排行榜-遂戏盒子`
      : `${category.name}游戏推荐_2026最新${category.name}手游排行榜-游戏盒子`,
    description: category.description || (isTW
      ? `精選${category.name}手遂戲排行榜與熱門推薦，提供快速下載與分類瀏覽服務。`
      : `精选${category.name}手游排行榜与热门推荐，提供快速下载与分类浏览服务。`),
    keywords: isTW
      ? [`${category.name}遂戏`, `${category.name}手遂戲`, `${category.name}推薦`, `${category.name}排行榜`, `${category.name}攻略`].join(',')
      : [`${category.name}游戏`, `${category.name}手游`, `${category.name}推荐`, `${category.name}排行榜`, `${category.name}攻略`].join(','),
    openGraph: {
      title: isTW
        ? `${category.name}遂戏推薦_2026最新${category.name}手遂戲排行榜`
        : `${category.name}游戏推荐_2026最新${category.name}手游排行榜`,
      description: category.description || (isTW
        ? `精選${category.name}手遂戲排行榜與熱門推薦`
        : `精选${category.name}手游排行榜与热门推荐`),
      url: catUrl,
    ...
    twitter: {
      card: 'summary',
      title: isTW
        ? `${category.name}遂戏推薦 | 遂戏盒子`
        : `${category.name}游戏推荐 | 游戏盒子`,
    },
    alternates: {
      canonical: catUrl,
      languages: (() => {
        const langs: Record<string, string> = { 'x-default': `${siteConfig.hostname}/games/category/${slug}` }
        for (const l of supportedLocales) {
          const prefix = l === defaultLocale ? '' : `/${l}`
          langs[l] = `${siteConfig.hostname}${prefix}/games/category/${slug}`
        }
        return langs
      })(),
    },
...
export default async function GameCategoryPage({ params, searchParams }: PageProps) {
  ...
  const prefix = localeTyped === defaultLocale ? '' : `/${localeTyped}`
  const categoryUrl = `${siteConfig.hostname}${prefix}/games/category/${slug}`
  const collectionSchema = buildCollectionPageSchema({
    name: category.name,
    description: category.description || category.name,
    url: categoryUrl,
    items: firstPageData.games?.slice(0, 10).map((g: any) => ({
      name: g.name,
      url: `${siteConfig.hostname}${prefix}/games/${g.id}`,
      image: g.iconUrl,
    })) ?? [],
  })
  const breadcrumbSchema = buildBreadcrumbSchema([
    { name: getTranslation('home', localeTyped), url: siteConfig.hostname },
    { name: getTranslation('gameLibrary', localeTyped), url: `${siteConfig.hostname}${prefix}/games` },
