...
import { JsonLd, buildGameSchema, buildBreadcrumbSchema } from '@/components/common/JsonLd'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const gameUrl = `${siteConfig.hostname}${locale === defaultLocale ? gamePath : `/${locale}${gamePath}`}`
  // canonical 指向主站同页面，避免与主站游戏详情页内容重复判重
  const mainSiteCanonical = `${siteConfig.mainSiteUrl}${gamePath}`
  const imageUrl = game.coverUrl || game.iconUrl || '/default-og-image.jpg'
  const description = game.description || `${t.metaDescPrefix}${game.name}${t.metaDescSuffix}`

  return {
    title: `${game.name}${t.metaTitleSuffix}`,
    description,
    keywords: game.tagList?.join(', '),
    openGraph: {
      title: game.name,
      description,
      url: gameUrl,
...
export default async function GameDetailPage({ params }: PageProps) {
  ...
  const gameUrlFull = `${siteConfig.hostname}${locale === defaultLocale ? '' : `/${locale}`}/games/${id}`

  const gameSchema = buildGameSchema({
    name: game.name,
    description: game.description,
    url: gameUrlFull,
    imageUrl: game.coverUrl || game.iconUrl,
    rating: game.rating,
  ...
  const breadcrumbSchema = buildBreadcrumbSchema([
    ...
    { name: t.gameLibrary, url: `${siteConfig.hostname}${locale === defaultLocale ? '' : `/${locale}`}/games` },
    { name: game.name, url: gameUrlFull },
  ])
