            ...
            <el-row>
               ...
               <el-col :span="24">
                  <el-form-item label="cron表达式" prop="cronExpression">
                     <el-input v-model="form.cronExpression" placeholder="格式：秒 分 时 日 月 周，如 0 0 8 * * ?">
                        <template #append>
                     ...
                     <div style="font-size: 12px; color: #909399; margin-top: 4px;">
                       格式: 秒 分 时 日 月 周&nbsp;&nbsp;
                       示例: 每早表8点<code style="background:#f5f7fa;padding:0 4px">0 0 8 * * ?</code>&nbsp;
                       每小时<code style="background:#f5f7fa;padding:0 4px">0 0 * * * ?</code>&nbsp;
                       每5分钟<code style="background:#f5f7fa;padding:0 4px">0 */5 * * * ?</code>
                     </div>
                  </el-form-item>
               </el-col>
    ...
    cronExpression: [
      { required: true, message: "cron执行表达式不能为空", trigger: "change" },
      {
        validator: (rule, value, callback) => {
          ...
          // Quartz cron 要求 6～7 个空格分隔的字段
          const parts = value.trim().split(/\s+/);
          if (parts.length < 6 || parts.length > 7) {
            callback(new Error('Cron 表达式必须为 6 个字段（秒 分 时 日 月 周），当前为 ' + parts.length + ' 个'));
            return;
          }
          callback();
        },
